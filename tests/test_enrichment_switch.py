...
from zettelforge.config import ZettelForgeConfig, _apply_yaml, get_config, reload_config
...
def test_enrichment_yaml_override():
    cfg = ZettelForgeConfig()
    _apply_yaml(cfg, {"enrichment": {"enabled": False}})
    assert cfg.enrichment.enabled is False
