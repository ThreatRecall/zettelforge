...
def test_low_signal_filter_counts_note_fanout_only(tmp_path):
    mm = _manager(tmp_path, "osint-fanout")
    wallet = "0x" + "ab" * 20
    for i in range(12):
        mm.store.add_kg_edge(
            "CryptoWallet",
            wallet,
            "Transaction",
            f"0x{i:064x}",
            "sent_transaction",
            properties={"edge_type": "osint"},
        )
    ...
    assert filtered == {"CryptoWallet": [wallet]}
