...
def test_recent_notes_by_domain_orders_and_limits(tmp_path):
    ...
    mm = MemoryManager(
        jsonl_path=str(tmp_path / 'n.jsonl'), lance_path=str(tmp_path / 'v')
    )
    for i in range(5):
        mm.remember(f'cti note {i}', source_type='threat_report',
                    source_ref=f'c{i}', domain='cti')
    for i in range(3):
        mm.remember(f'general note {i}', source_type='conversation',
                    source_ref=f'g{i}', domain='general')
...
def test_defense_gate_receives_bounded_reference_set(tmp_path, monkeypatch):
    ...
    for i in range(6):
        mm.remember(f'note {i}', source_type='conversation',
                    source_ref=f's{i}', domain='general')
